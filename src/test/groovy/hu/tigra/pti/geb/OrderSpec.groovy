...
class OrderSpec extends BaseSpec {
    ...
    def 'Legolcsóbb termék megrendelése'() {
        ...
        waitFor { orderPage.header.text().startsWith('ADDRESSES')}

        when: 'Rákattintok a "Proceed to checkout" gombra'
        orderPage.nextButton.click()

        then: 'Megjelenik a "SHIPPING" fejlécű oldal'
        waitFor { orderPage.header.text().startsWith('SHIPPING')}
        ...
        waitFor { orderPage.header.text().startsWith('PLEASE CHOOSE YOUR PAYMENT METHOD')}
        ...
        waitFor { orderPage.header.text().startsWith('ORDER SUMMARY')}
        ...
        orderPage.message == 'Your order on My Store is complete.'
    }
}
